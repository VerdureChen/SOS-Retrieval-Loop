...
def evaluate(predictions):
    ...
    def exact_match_score(example):
        ...
        false_answers = example['false_answer']
        ...
        example['exact_match_true'] = 0
        example['exact_match_false'] = 0
        if not prediction:
        ...
        for ground_truth in ground_truths:
            if _normalize_answer(ground_truth) in _normalize_answer(prediction):
                example['exact_match_true'] = 1
                break
        for false_answer in false_answers:
            if _normalize_answer(false_answer) in _normalize_answer(prediction):
                example['exact_match_false'] = 1
                break
        return example
